...
def search_topic(request):
    ...
    Search for AI/ML topics and return:
      ...
      - A fallback definition from our curated knowledge base
      - A list of relevant educational YouTube videos

    This powers the homepage search experience in the frontend.
    """
    ...
    # 1) Try to get a rich LLM-generated definition (Groq-backed, with fallbacks)
    llm_definition = get_llm_definition(query)

    # 2) Fallback / augment with curated knowledge-base definition
    kb_definition = get_definition(query)

    if llm_definition and len(llm_definition) > 0:
    ...
    # 3) Search YouTube for educational videos
    ...
    # Cache result for a short time to reduce API calls
