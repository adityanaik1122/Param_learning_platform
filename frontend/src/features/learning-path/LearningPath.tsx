...
import { phase1MLContent } from './ai-ml';
import { phase2MathContent } from './ai-ml';
import { phase3PythonContent } from './ai-ml';
import { phase4ClassicalMLContent } from './ai-ml';
import { phase5DeepLearningContent } from './ai-ml';
import { phase6NLPContent } from './ai-ml';
import { phase7GenAILLMContent } from './ai-ml';
import { phase8ComputerVisionContent } from './ai-ml';
import { phase9AgenticAIContent } from './ai-ml';
import { phase10MLOpsProductionContent } from './ai-ml';
import { designEngineeringContent } from './design-engineering';
import { gameDevelopmentPhases } from './game-development';
import { courseProgressAPI } from '../../services/courseProgressApi';

export default function LearningPath() {
  const navigate = useNavigate();
  const location = useLocation();const { courseId } = useParams();
  ...
  useEffect(() => {
    const fetchCurrentCourse = async () => {
      try {
        ...
        const current = progressData.find(p => p.is_current);
        if (current) {
          setCurrentCourse(current.id);
        }
      } catch (error) {
    ...
    if (routeCourseId) {
      setCurrentCourse(routeCourseId);
      setCourseLoading(false);
      return;
    }
...
`.replace(/\$/g, '\\$')
  ...
  const designEngineeringCurriculum = Array.isArray(designEngineeringContent)
    ? designEngineeringContent
    : [designEngineeringContent];
      ...
      {curriculum.map(phase => {
        ...
        return (
          <div key={phase.id} style={{ marginBottom: '1rem' }}>
            <div
              className={`plh-sidebar-card${isPhaseExpanded ? ' active' : ''}`}
              onClick={() => {
                if (isPhaseExpanded) {
      ...
      <CyberpunkLayout sidebar={sidebarContent} sidebarTitle="Curriculum" fullWidth>
        ...
        <div
          style={{
            ...
            background: 'linear-gradient(90deg, rgba(15,23,42,0.95) 0%, rgba(17,24,39,0.95) 40%, rgba(30,64,175,0.4) 100%)',
            display: 'flex',
            alignItems: 'center',
            gap: '0.5rem',
            fontSize: '0.8rem',
        ...
        >
          ...
          {currentPhase && (
            <>
              <span>/</span>
              <span style={{ color: '#c4b5fd' }}>{currentPhase.title}</span>
            </>
          )}
          {currentLesson && (
            <>
